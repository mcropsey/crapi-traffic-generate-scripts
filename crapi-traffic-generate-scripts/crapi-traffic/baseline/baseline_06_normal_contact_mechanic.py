#!/usr/bin/env python3
"""Baseline 6: Normal single contact mechanic request"""
...
def main():
    parser = argparse.ArgumentParser(description="Baseline 6: Normal Contact Mechanic")
    ...
    print(f"[*] Baseline 6: Normal Contact Mechanic\n[*] Target: {base_url}\n" + "-"*60)
    ...
    print(f"[*] Contacting mechanic (single normal request)...")
    result = contact_mechanic(base_url, token, vin)
    if result:
        print(f"[+] SUCCESS! Mechanic contacted")
        print(f"[+] BASELINE: Legitimate single mechanic contact")
    else:
