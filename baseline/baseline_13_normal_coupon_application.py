...
def main():
    ...
    if not orders:
        print("[+] User has no orders (skipping coupon application)")
        print("[+] BASELINE: Legitimate coupon system access")
        return
    ...
    if result:
        if "error" not in result.get("message", "").lower():
            ...
            print(f"[*] Coupon application: {result.get('message')}")
            print(f"[+] BASELINE: Legitimate coupon application attempt")
    else:
