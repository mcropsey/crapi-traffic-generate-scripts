#!/usr/bin/env python3
"""Baseline 8: Normal positive quantity purchase"""
...
def main():
    parser = argparse.ArgumentParser(description="Baseline 8: Normal Purchase")
    ...
    print(f"[*] Baseline 8: Normal Purchase\n[*] Target: {base_url}\n" + "-"*60)
    ...
    print(f"[*] Placing order: {product.get('name')} x{quantity}")

    order = place_order(base_url, token, product_id, quantity)
    if order:
        print(f"[+] Order placed successfully")
        print(f"[+] BASELINE: Legitimate purchase with positive quantity")
    else:
        print("[!] Order failed")
