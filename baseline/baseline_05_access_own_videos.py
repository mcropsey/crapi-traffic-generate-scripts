...
def main():
    ...
    print(f"[*] Accessing own video info from dashboard...")
    dashboard = get_dashboard(base_url, token)
    if dashboard and dashboard.get("video_id"):
        print(f"[+] Found video:")
        print(f"  - Video ID: {dashboard.get('video_id')}, Name: {dashboard.get('video_name')}")
