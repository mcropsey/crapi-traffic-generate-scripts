#!/usr/bin/env python3
"""
Generate NORMAL traffic for shop browsing and order history.
...
browse the shop and view their purchase history.
"""
...
def get_all_orders(base_url: str, token: str, timeout: float = 15) -> List[Dict[str, Any]]:
    """GET /workshop/api/shop/orders/all – list all orders for user."""
...
def get_order_details(base_url: str, token: str, order_id: int, timeout: float = 15) -> Optional[Dict[str, Any]]:
    """GET /workshop/api/shop/orders/{order_id} – get details of own order."""
    url = f"{base_url.rstrip('/')}/workshop/api/shop/orders/{order_id}"
    headers = {"Authorization": f"Bearer {token}"}
    try:
        r = requests.get(url, headers=headers, timeout=timeout)
        r.raise_for_status()
        ...
        print(f"  [!] get_order_details({order_id}) failed: {e}")
        return None


def main():
    parser = argparse.ArgumentParser(
        description="Generate normal (legitimate) shop browsing traffic for crAPI"
    )
    parser.add_argument(
        "--config",
    ...
    print(f"Purpose : NORMAL traffic only (own orders)")
    print("-" * 60)
    ...
    for cycle in range(1, args.cycles + 1):
        ...
        for idx, email in enumerate(emails):
            ...
            # 2. Browse products (normal shopping behaviour)
            ...
            # 3. View all orders
            orders = get_all_orders(base_url, token, timeout=args.timeout)
            if not orders:
            ...
            # 4. View details of first order only
            order = orders[0]
            order_id = order.get("id")
            if order_id:
                order_details = get_order_details(base_url, token, order_id, timeout=args.timeout)
                if order_details:
                    status = order_details.get("status") or order_details.get("id", "")
                    print(f"  Viewed order {order_id} (status: {status})")
                else:
                    print(f"  Failed to fetch order {order_id} details")
            else:
    ...
    print("\nDone – normal shop browsing traffic generated.")
